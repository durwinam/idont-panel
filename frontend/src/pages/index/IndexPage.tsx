...
export default function IndexPage() {
  ...
  const [panelUpdateInfo, setPanelUpdateInfo] = useState({
    currentVersion: '',
    latestVersion: '',
  ...
  useEffect(() => {
    ...
    }>('/panel/api/setting/defaultSettings').then((msg) => {
  ...
  async function copyConfig() {
    const ok = await ClipboardManager.copyText(
      configText || '',
    );
  ...
  const freeDisk = Math.max(
    0,
    totalDisk - status.disk.current,
  );

  const health = useMemo(() => {
    const items = [
      {
        name: t('pages.index.cpu'),
        value: status.cpu.percent,
      },
      {
        name: t('pages.index.memory'),
        value: status.mem.percent,
      },
      {
        name: t('pages.index.swap'),
        value: status.swap.percent,
      },
      {
        name: t('pages.index.storage'),
        value: status.disk.percent,
      },
    ];

    const list = (xs: typeof items) =>
      xs
        .map(
          (i) =>
            `${i.name} ${i.value.toFixed(0)}%`,
        )
        .join(', ');

    const crit = items.filter(
      (i) => i.value >= USAGE_CRIT_PERCENT,
    );

    if (crit.length) {
      return {
        text: t(
          'pages.index.healthCritical',
          { list: list(crit) },
        ),
    ...
    const warm = items.filter(
      (i) => i.value >= USAGE_WARN_PERCENT,
    );

    if (warm.length) {
      return {
        text: t(
          'pages.index.healthWarm',
          { list: list(warm) },
        ),
  ...
  return (
    <ConfigProvider theme={antdThemeConfig}>
      <div className={pageClass}>
        ...
        <Layout className="content-shell ios-glass-shell">
          <Layout.Content className="content-area ios-glass-content">
            <Spin
            ...
            >
              {!fetched ? (
                <div className="loading-spacer ios-glass-loading" />
              ) : fetchError ? (
                <div className="ios-glass-error">
                  <Result
                    status="error"
                ...
                <div className="ov-page ios-glass-dashboard">

                  {/* ACTION BAR */}
                  <section className="ios-glass-panel ios-glass-action-panel">
                    <OverviewActionBar
                      status={status}
                      isMobile={isMobile}
                  ...
                  {health && (
                    <div
                      className="ov-health ios-glass-health"
                      style={{
                        color: health.color,
                      }}
                    >
                  ...
                  {/* SEPARATOR */}
                  <div className="ov-rule ios-glass-rule" />

                  {/* VITALS */}
                  <section className="ov-vitals ios-glass-vitals">
                    ...
                    <div className="ios-glass-card">
                      <VitalTile
                        ...
                        detail={`${CPUFormatter.cpuCoreFormat(
                          status.cpuCores,
                        )} / ${
                          status.logicalPro
                        }T · ${CPUFormatter.cpuSpeedFormat(
                          status.cpuSpeedMhz,
                        )}`}
                        footLeft={`${t(
                          'pages.index.avg',
                    ...
                    <div className="ios-glass-card">
                      <VitalTile
                        ...
                        detail={`${SizeFormatter.sizeFormat(
                          status.mem.current,
                  ...
                  {/* NETWORK */}
                  <section className="ov-mid ios-glass-network">

                    <div className="ios-glass-card ios-glass-chart-card">
                      <ThroughputCard
                        status={status}
                  ...
                  {/* SYSTEM */}
                  <section className="ios-glass-card ios-glass-system">
                    <SystemStrip
                      status={status}
                      showIp={showIp}
                      onToggleIp={() =>
          ...
          <Modal
            ...
            footer={[
              ...
              <Button
                ...
                className="ios-glass-primary-button"
              >
                Copy
              </Button>,
            ]}
          >
            <div className="ios-glass-editor">
              <JsonEditor
                value={configText}
